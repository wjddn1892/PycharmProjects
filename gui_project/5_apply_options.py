...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요",
                                        ...
                                        initialdir=r"C:\Users\관리자\PycharmProjects")
                                        # 최초에 사용자가 지정한 경로를 보여줌      r을 이용하여 문자 그대로 사용함
    # 사용자가 선택한 파일 목록
    for file in files:
...
# 선택 삭제
def del_file():
    # print(list_file.curselection())   # 선택한 것의 index를 알려줌
    for index in reversed(list_file.curselection()):    # reversed : 원래 리스트는 뒤집히지 않고 뒤집힌 것을 다른 변수에 리턴
...
def browse_dest_path():
    ...
    txt_dest_path.delete(0, END)    # entry 이므로 0, END  textwidget 이면 "1.0", END
...
def merge_image():
    ...
    try:
        ...
        if img_width == "원본유지":
            ...
            img_width = int(img_width)  # 문자열 형태였으므로
        ...
        # 계산식
        # 100 * 60 이미지가 있음 -> width를 80으로 줄ㅇ면 height는?
        ...
        widths, heights = zip(*(image_sizes))    # 위의 코드를 변형
