...
def shuffle_grid(number_count):
    ...
    cell_size = 130 # grid cell 별 가로, 세로 크기
...
def check_buttons(pos):
    global start, start_ticks  # 바깥의 start 변수를 가져와 사용하겠다
...
while running:
    ...
    for event in pygame.event.get():  # 어떤 이벤트가 발생하였는가?
        ...
        elif event.type == pygame.MOUSEBUTTONUP:  # 사용자가 마우스를 클릭했을 떄
    ...
    if click_pos:
        check_buttons(click_pos)    # ctrl 누른 채로 클릭하면 그 함수가 정의된 곳으로 이동
